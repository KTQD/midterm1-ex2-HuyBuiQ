...
public class Main {
    public static Map<String, Integer> demtutrongcau(String text){
        Map<String, Integer> demTu = new HashMap<>();
        String[] danhsachtu = text.split("\\s+");
        for (String tu : danhsachtu){
            tu = tu.toLowerCase();
            if (demTu.containsKey("word")){
                demTu.put(tu,demTu.get(tu)+1);
            }
            else{
                demTu.put(tu,1);
            }
    ...
    Map<String, Integer> demTu = demtutrongcau(chuoi_text);
    for (String tu : demTu.keySet()) {
        System.out.println(tu + ": "+demTu.get(tu));
    }
